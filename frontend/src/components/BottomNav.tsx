...
export function BottomNav() {
  ...
  return (
    <nav className="flex flex-none items-center justify-around border-t border-separator/60 bg-surface pb-safe sm:pb-0 sm:rounded-b-ios-sheet">
      {NAV_ITEMS.map((item) => {
          ...
          >
            ...
            {isActive && (
              <motion.div
                ...
                className="absolute inset-x-3 inset-y-1 rounded-ios bg-brand/12"
              />
            )}
            ...
            <Icon
              ...
              className={`relative z-10 transition-colors ${
                isActive ? "text-brand" : "text-label-tertiary/70"
              }`}
            />
            <span
              className={`relative z-10 text-caption2 transition-colors ${
                isActive ? "font-semibold text-brand" : "text-label-tertiary/70"
              }`}
            >
              {item.label}
            </span>
